...
describe 'check_mk::config', type: :class do
  context 'with site set' do
    ...
    it {
      is_expected.to contain_concat__fragment('all-hosts-static').with(ensure: '/omd/sites/TEST_SITE/etc/check_mk/all_hosts_static',
    ...
    it { is_expected.not_to contain_check_mk__hostgroup }
    it {
      is_expected.to contain_concat__fragment('check_mk-local-config').with(ensure: '/omd/sites/TEST_SITE/etc/check_mk/main.mk.local',
                                                                            target: '/omd/sites/TEST_SITE/etc/check_mk/main.mk',
                                                                            order: 99)
    }
    it {
