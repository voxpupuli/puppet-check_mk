  ...
  on_supported_os.each do |os, facts|
    context "on #{os}" do
      ...
      context 'with hostgroups, host_tags and description' do
        ...
        it {
          expect(subject).to contain_concat__fragment('check_mk-hostgroup-TEST_HOSTGROUP').with(target: 'target',
        ...
        EOS
        it {
          expect(subject).to contain_file('/dir/TEST_HOSTGROUP.cfg').with(ensure: 'file',
      ...
      context 'with hostgroups without description' do
        ...
        it {
          expect(subject).to contain_concat__fragment('check_mk-hostgroup-TEST_HOUSTGROUP_WITH_UNDERSCORES').with(target: '/target',
        ...
        EOS
        it {
          expect(subject).to contain_file('/dir/TEST_HOUSTGROUP_WITH_UNDERSCORES.cfg').with(ensure: 'file',
                                                                                            content: expected_file_content)
        }
      end
    end
