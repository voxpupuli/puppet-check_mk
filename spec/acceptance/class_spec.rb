...
describe 'check_mk class' do
  packagename = case fact('os.family')
                when 'Debian'
                  "check-mk-raw-2.0.0p36_0.#{fact('os.distro.codename')}_amd64.deb"
                when 'RedHat'
                  "check-mk-raw-2.0.0p36-el#{fact('os.release.major')}-38.x86_64.rpm"
                end
  ...
  context 'minimal parameters' do
    ...
    it 'works idempotently with no errors' do
      pp = <<-EOS
      class { 'check_mk':
        filestore => 'https://download.checkmk.com/checkmk/2.0.0p36/',
        package   => '#{packagename}',
      }
      EOS
